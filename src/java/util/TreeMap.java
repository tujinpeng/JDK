    ...
    implements NavigableMap<K,V>, Cloneable, java.io.Serializable
{
    ...
    private void fixAfterInsertion(Entry<K,V> x) {
         ...
         *   红新
         *
         *  若插入节点的父节点是黑色,则树自平衡,不需要再调整(红黑树比平衡二叉树AVL优秀的地方在于:红黑树中黑父的节点比较多,从而需要平衡操作的几率大大减少)
         */
        /*
         *2.红父:
         *     黑祖父
         ...
         *   红新
         *
         *  若插入节点的父节点是红色,违反红黑树性质4,祖父节点一定是黑色,下面需要根据叔叔节点的颜色进行平衡操作
         */
        while (x != null && x != root && x.parent.color == RED) {
            ...
            if (parentOf(x) == leftOf(parentOf(parentOf(x)))) {
                ...
                if (colorOf(y) == RED) {
                    ...
                    //此时祖父节点变为红色，祖父节点以上可能不平衡，此时要将祖父节点变成新的判定点继续向上平衡
                     ...
                     *  LR型:
                     *      黑祖父                                        黑祖父                                            黑新
                     *      /  \        先以红父为中心左旋，转为LL型         /   \          在以黑祖父为中心右旋                  /   \
                     *    红父  黑叔  ------------------------------->  红新  黑叔  -------------------------------->      红父   红祖父
                     *    /  \                                        /   \            红新和黑祖节点颜色交换              / \     /  \
                     *   N1  红新                                     红父  N3                                          N1 N2  N3   黑叔
                     *       /  \                                    /  \
                     *      N2   N3                                 N1   N2
                     */
    ...
    private void fixAfterDeletion(Entry<K,V> x) {
        ...
        /*
         * 2.删除叶子节点(黑色x):
         *   父节点到x位置少了一个黑色,x位置以上失去平衡（不满足红黑树性质5）,要判断兄弟节点的颜色来进行平衡操作
         */
        while (x != root && colorOf(x) == BLACK) {
            ...
            if (x == leftOf(parentOf(x))) {
                ...
                if (colorOf(leftOf(sib))  == BLACK &&
                     ...
                     * 2.2.1 黑兄二黑侄:                                                            父
                     *     父                                                                    /  \
                     *    /  \          父节点左边少了一个黑色，为了平衡要将黑兄改为红色节点              x   红兄
                     *   x   黑兄      ----------------------------------------------------->        / \
                     *       /  \             此时父节点以下保持平衡了                               黑侄 黑侄
                     *     黑侄 黑侄
                     *                                                                            父   <--- x(不平衡点)
                     *        由于父节点到右边叶子节点少了一个黑色，父节点以上到父节点变得不平衡              /  \
                     *     --------------------------------------------------------------->     就x 红兄
                     *                 将不平衡位置引用x指向父节点                                    /  \
                     *                                                                         黑侄 黑侄
                     */
                     ...
                     * 2.2.2 黑兄-左红侄-右黑侄:                                                       父
                     *     父                                                                      /  \
                     *    /  \             RL型:先黑兄和左红侄颜色互换,然后以黑兄为中心右旋(R)            x  黑左侄  <-----sib(x的新兄弟节点)
                     *   x   黑兄      ----------------------------------------------------->        /   \
                     *       /  \                                                                   y  红兄
                     *    红左侄 黑右侄                                                                  /  \
                     *     / \                                                                        j  黑右侄
                     ...
                     *
                     *
                     *   然后将新sib节点和父节点颜色互换,父节点颜色变为黑色,新sib的右侄变为黑色               左侄(颜色和原父颜色一致)
                     *  --------------------------------------------------------------->         /   \
                     *                  以父节点为中心左旋(L)                                      黑父   黑兄
                     *                                                                          / \   /  \
                     *                                                                         x   y j  黑右侄
                    ...
                    /*
                     * 2.2.3 黑兄-左黑侄-右红侄:                                                                     兄(颜色和父节点颜色一致)
                     *     父                                                                                    /  \
                     *    /  \             RR型:然后将sib节点和父节点颜色互换,父节点颜色变为黑色,sib的右侄变为黑色         黑父  右黑侄
                     *   x   黑兄      ---------------------------------------------------------------->        / \
                     *       /  \                           以父节点为中心左旋(L)                                 x  左黑侄
                     *    左黑侄 右红侄
                     *     / \
                     ...
                     *
                     */
                    //兄弟节点和父节点颜色互换(保持兄弟节点旋转成为新父节点时,新父以上平衡)
                    setColor(sib, colorOf(parentOf(x)));
                    //父节点和右侄节点设置成黑色,保持新父节点以下平衡
                    setColor(parentOf(x), BLACK);
